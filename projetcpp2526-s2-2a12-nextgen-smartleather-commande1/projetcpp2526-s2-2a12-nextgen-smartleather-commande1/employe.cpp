...
void EmployeWindow::populateTable()
{
    ...
    QSqlDatabase db = Connection::instance()->database();
    QSqlQuery query(db);
    query.prepare("SELECT CIN_EMPLOYE, NOM, POSTE, ADRESSE, TELEPHONE, DATE_EMBAUCHE, SALAIRE, STATUT, SEXE FROM EMPLOYE ORDER BY CIN_EMPLOYE");
...
void EmployeWindow::addEmployee()
{
    ...
    checkQuery.prepare("SELECT 1 FROM EMPLOYE WHERE CIN_EMPLOYE = :cin");
    ...
    QSqlQuery query(db);
    query.prepare(
        "INSERT INTO EMPLOYE (CIN_EMPLOYE, NOM, POSTE, ADRESSE, TELEPHONE, DATE_EMBAUCHE, SALAIRE, STATUT, SEXE) "
...
void EmployeWindow::loadEmployee()
{
    ...
    QSqlDatabase db = Connection::instance()->database();
    QSqlQuery query(db);
    query.prepare("SELECT NOM, TELEPHONE, SALAIRE FROM EMPLOYE WHERE CIN_EMPLOYE = :cin");
    query.bindValue(":cin", cin);
...
void EmployeWindow::updateEmployee()
{
    ...
    QSqlDatabase db = Connection::instance()->database();
    QSqlQuery query(db);
    query.prepare("UPDATE EMPLOYE SET NOM = :nom, TELEPHONE = :telephone, SALAIRE = :salaire WHERE CIN_EMPLOYE = :cin");
    query.bindValue(":nom", name);
    query.bindValue(":telephone", phone);
    query.bindValue(":salaire", salary);
    query.bindValue(":cin", cin);
