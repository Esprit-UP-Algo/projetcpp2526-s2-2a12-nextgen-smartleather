...
namespace {
constexpr bool kSmtpEnabled = false; // toggle SMTP sending
    ...
    , ui(new Ui::MainWindow)
{
    ...
    */
    ...
    if (ui->btn_update_confirm) {
        ui->btn_update_confirm->setMinimumHeight(36);
    }
    ...
    // Créer et configurer un timer pour les notifications
    // QTimer *notificationTimer = new QTimer(this);
    // connect(notificationTimer, &QTimer::timeout, this, &MainWindow::checkForNotifications);
    // notificationTimer->start(3600000); // Vérifier toutes les heures (3600000 ms) // DÉSACTIVÉ

    // Initialisation
    reloadOrdersFromDb();
    onCalendarDateChanged();
    updateStatistics();
    // checkForNotifications(); // Vérification initiale // DÉSACTIVÉ
...
void MainWindow::sortOrdersList()
{
    ...
    // Créer une liste de toutes les données pour un tri personnalisé
    struct RowData {
        QString id, type, email, ville, dateCmd, dateLivr, statut;
        int quantite;
        double prix;
        QString cinEmploye;
    };
    ...
    for (int row = 0; row < ui->table_list->rowCount(); ++row) {
        RowData data;
        data.id = ui->table_list->item(row, 0)->text();
        data.type = ui->table_list->item(row, 1)->text();
        data.quantite = ui->table_list->item(row, 2)->text().toInt();
        data.email = ui->table_list->item(row, 3)->text();
        data.ville = ui->table_list->item(row, 4)->text();
        data.dateCmd = ui->table_list->item(row, 5)->text();
        data.dateLivr = ui->table_list->item(row, 6)->text();
        data.prix = ui->table_list->item(row, 7)->text().toDouble();
        data.cinEmploye = ui->table_list->item(row, 8)->text();
        data.statut = ui->table_list->item(row, 9)->text();
        
        allRows.append(data);
    }
    
    // Trier selon la colonne et l'ordre choisis
    std::sort(allRows.begin(), allRows.end(), [column, order](const RowData &a, const RowData &b) {
        bool result = false;
        
        switch (column) {
            case 0: // ID
                result = a.id.toInt() < b.id.toInt();
                break;
            case 1: // Type
                result = a.type < b.type;
                break;
            case 2: // Quantité
                result = a.quantite < b.quantite;
                break;
            case 3: // Email
                result = a.email < b.email;
                break;
            case 4: // Ville
                result = a.ville < b.ville;
                break;
            case 5: // Date Commande
                result = a.dateCmd < b.dateCmd;
                break;
            case 6: // Date Livraison
                result = a.dateLivr < b.dateLivr;
                break;
            case 7: // Prix
                result = a.prix < b.prix;
                break;
            case 9: // Statut
                result = a.statut < b.statut;
                break;
            default:
                result = a.id.toInt() < b.id.toInt();
        }
    ...
    // Réappliquer les données triées dans la table
    ui->table_list->setRowCount(0);
    
    for (const RowData &data : allRows) {
        int row = ui->table_list->rowCount();
        ui->table_list->insertRow(row);
        ui->table_list->setItem(row, 0, new QTableWidgetItem(data.id));
        ui->table_list->setItem(row, 1, new QTableWidgetItem(data.type));
        ui->table_list->setItem(row, 2, new QTableWidgetItem(QString::number(data.quantite)));
        ui->table_list->setItem(row, 3, new QTableWidgetItem(data.email));
        ui->table_list->setItem(row, 4, new QTableWidgetItem(data.ville));
        ui->table_list->setItem(row, 5, new QTableWidgetItem(data.dateCmd));
        ui->table_list->setItem(row, 6, new QTableWidgetItem(data.dateLivr));
        ui->table_list->setItem(row, 7, new QTableWidgetItem(QString::number(data.prix)));
        ui->table_list->setItem(row, 8, new QTableWidgetItem(data.cinEmploye));
        ui->table_list->setItem(row, 9, new QTableWidgetItem(data.statut));
    }
    
    // Réappliquer le filtre de recherche si actif
    searchOrdersList();
    
    qDebug() << "Tri appliqué sur la liste:" << sortType;
}
