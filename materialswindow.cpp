...
static QString gMaterialsTableName = "MATIERE_PREMIERE";
    ...
    , ui(new Ui::MaterialsWindow)
{
    ...
    if (ui->le_upd_search_name) {
        ui->le_upd_search_name->setMinimumWidth(500);
    ...
    // Assurer la visibilité des actions de modification, même quand la place est réduite
    if (ui->top_nav) {
    ...
    if (ui->btn_upd_load) {
        ui->btn_upd_load->setMinimumWidth(160);
        ui->btn_upd_load->setMaximumWidth(180);
    ...
    if (ui->btn_upd_confirm) {
        ui->btn_upd_confirm->setMinimumHeight(fieldHeight);
    ...
    if (ui->btn_upd_load) {
        ...
        ui->btn_upd_load->setMinimumHeight(fieldHeight);
        ui->btn_upd_load->setMinimumWidth(140);
        ui->btn_upd_load->setSizePolicy(QSizePolicy::Fixed, QSizePolicy::Fixed);
        ui->btn_upd_load->setStyleSheet("background-color: #8B4513; color: white; border-radius: 10px; padding: 12px 26px; font-weight: bold; border: none;");
        ui->btn_upd_load->show();
    }
    ...
    auto setUpdateEnabled = [&](bool enabled) {
    ...
    QList<QPushButton*> navButtons = { ui->btn_tab_add, ui->btn_tab_update, ui->btn_tab_delete, ui->btn_tab_list, ui->btn_tab_stats };
    for (auto *btn : navButtons) {
        if (!btn) continue;
        navGroup->addButton(btn);
        btn->setCheckable(true);
    }
    auto setNavChecked = [&](QPushButton *active){
        for (auto *btn : navButtons) {
            if (!btn) continue;
...
void MaterialsWindow::refreshStats()
{
    ...
    const int avgYield = (total > 0) ? static_cast<int>(qBound(0.0, totalYield / total, 100.0)) : 0;
    const int avgLossInt = static_cast<int>(qBound(0.0, avgLoss, 100.0));
    ui->pb_yield->setRange(0, 100);
    ui->pb_loss->setRange(0, 100);
    ui->pb_yield->setValue(avgYield);
    ui->pb_loss->setValue(avgLossInt);
    ui->pb_yield->setFormat(QString("%1 %").arg(avgYield));
    ui->pb_loss->setFormat(QString("%1 %").arg(avgLossInt));
